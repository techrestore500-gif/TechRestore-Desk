...
def get_history(_: dict = Depends(require_role("admin", "manager"))) -> list[SystemActivityResponse]:
    return SystemService.list_history()


@router.get("/export")
def export_snapshot(_: dict = Depends(require_role("admin", "manager"))) -> Response:
...
def get_loaner_agreement_defaults(_: dict = Depends(require_role("admin", "manager"))) -> LoanerAgreementDefaultsResponse:
...
def get_notification_templates(_: dict = Depends(require_role("admin", "manager"))) -> list[NotificationTemplate]:
...
def post_scan_overdue_loaners(_: dict = Depends(require_role("admin", "manager"))) -> dict:
...
def get_query_metrics(_: dict = Depends(require_role("admin", "manager"))) -> QueryMetricsResponse:
    return SystemService.get_query_metrics()


@router.post("/performance/query-metrics/reset")
def post_reset_query_metrics(_: dict = Depends(require_role("admin", "manager"))) -> dict:
    return SystemService.reset_query_metrics()


@router.get("/runtime-diagnostics", response_model=RuntimeDiagnosticsResponse)
def get_runtime_diagnostics(_: dict = Depends(require_role("admin", "manager"))) -> RuntimeDiagnosticsResponse:
