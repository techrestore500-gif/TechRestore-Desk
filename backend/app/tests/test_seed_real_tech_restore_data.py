...
def test_real_data_seed_script_replaces_demo_ticket_and_hours_data_without_touching_protected_tables(test_db):
    ...
    assert summary.tickets_deleted >= 1
    assert summary.hours_deleted >= 1
    assert summary.active_clock_sessions_deleted == 0
    assert summary.tickets_inserted == 4
    assert summary.hours_inserted == 10
    assert summary.pricing_notes_inserted >= 4
    ...
    with database.get_connection() as connection:
        ...
        total_hours = connection.execute("SELECT ROUND(SUM(hours_worked), 2) FROM technician_hours").fetchone()[0]
        zero_hour_entries = connection.execute("SELECT COUNT(*) FROM technician_hours WHERE hours_worked <= 0").fetchone()[0]
        may_11_entries = connection.execute("SELECT COUNT(*) FROM technician_hours WHERE work_date = '2026-05-11'").fetchone()[0]
        ...
        yossi_weiss = connection.execute(
            """
            SELECT rt.final_price, rt.payment_status
    ...
    assert hours_count == 10
    assert total_hours == 14.0
    assert zero_hour_entries == 0
    assert may_11_entries == 0
    ...
    assert yossi_weiss["final_price"] == 25.0
    assert yossi_weiss["payment_status"] == "unpaid"
